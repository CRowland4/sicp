...
(parallel-execute
 (ser (lambda () (set! y (* y y))))
 (ser (lambda () (set! y (+ y 1)))))
...
; Example of a procedure with more than one shared resource
...
; A version of make-account that exposes the serializer via message-passing
...
; Explicitly managing serialization of account procedures
(define (deposit account amount)
  (let ((s (account 'serializer))
        (d (account 'deposit)))
    ((s d) amount)))

; Implementing a serialized exchange procedure
